...
type CategoryRepository interface {
	SelectOne(ctx context.Context, category *model.Category, col string) error
	SelectMany(ctx context.Context, category *model.Categories, col string) error
	InsertOne(ctx context.Context, category *model.Category) error
	InsertMany(ctx context.Context, category *model.Categories) error
	UpdateOne(ctx context.Context, category *model.Category) error
	DeleteOne(ctx context.Context, category *model.Category, col string) error
...
func (a *categoryRepositoryAgent) SelectOne(ctx context.Context, category *model.Category, col string) error {
	...
	if err := a.db.NewSelect().Model(category).WherePK(col).Scan(ctx); err != nil {
...
func (a *categoryRepositoryAgent) SelectMany(ctx context.Context, category *model.Categories, col string) error {
...
func (a *categoryRepositoryAgent) InsertMany(ctx context.Context, category *model.Categories) error {
...
func (a *categoryRepositoryAgent) UpdateMany(ctx context.Context, category *model.Categories) error {
	ctx, span := otel.Tracer("category-repository").Start(ctx, "update-many")
	defer span.End()
	a.logger.For(ctx).Info("Updating many category")

	values := a.db.NewValues(category)
...
func (a *categoryRepositoryAgent) DeleteOne(ctx context.Context, category *model.Category, col string) error {
	ctx, span := otel.Tracer("category-repository").Start(ctx, "delete-one")
	defer span.End()
	a.logger.For(ctx).Info("Hard deleting category")

	_, err := a.db.NewDelete().Model(category).Where("? = ?", bun.Ident(col), category.Id).Exec(ctx)
	if err != nil {

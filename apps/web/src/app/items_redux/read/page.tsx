...
import { useAppSelector, useAppDispatch, useAppStore } from '@/lib/state/hook';
// import {
//   initializeProduct,
//   setProductName,
//   Product,
// } from '@/lib/state/items/itemsSlice';
...
import { ForwardArrow, BackwardArrow } from '@/components/PaginationArrows';
...
function getComps(items, recordsPerPage, page, maxPages, handlePageChange): ReactNode{
...
export default function ItemsRead(): ReactNode {
  ...
  const recordsPerPage = useAppSelector((state) => state.items.pageRecordCount);
  const page = useAppSelector((state) => state.items.currentPage);
  const storeData = useAppSelector((state) => state.items.pageMap.get(page));
  const dispatch = useAppDispatch();


  const userId = "4d56128c-5042-4081-a0ef-c2d064700191";
  const category: string[] = useAppSelector((state) => state.items.categoryFilter);
  const tags: string[] = useAppSelector((state) => state.items.tagFilter);
  const totalRecords: number = useAppSelector((state) => state.items.totalRecordCount);
  const maxPages = Math.ceil(totalRecords / recordsPerPage);


  function handlePageChange(event: React.ChangeEvent<unknown>, value: number) {
    dispatch(updateItemsCurrentPage(value));
  };

  console.info('Stored Items');
  ...
  if (typeof storeData === "undefined"){
    ...
    // dispatch(updateItemsPageMap(data));
    return getComps(items, recordsPerPage, page, maxPages, handlePageChange);


  } else {
    return getComps(storeData, recordsPerPage, page, maxPages, handlePageChange);
  }
